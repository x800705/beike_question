<template>
	<view style="margin-top: 20px;">
		...
		<view class="cos">
			...
			<radio-group>
				<view> <input placeholder="选项A" v-model="a">
					<radio @click="gans(0)" :checked="radio_array[0]"></radio>
				</view>
				<view> <input placeholder="选项B" v-model="b">
					<radio @click="gans(1)" :checked="radio_array[1]"></radio>
				</view>
				<view> <input placeholder="选项C" v-model="c">
					<radio @click="gans(2)" :checked="radio_array[2]"></radio>
				</view>
				<view> <input placeholder="选项D" v-model="d">
		...
		methods: {
			...
			submit() {
				...
				if (this.ans === "") {
					...
					uni.request({
						url: this.url + "addOne", //仅为示例，并非真实接口地址。
						timeout: 2000,
						data: {
							que: this.que,
